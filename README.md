...
独立した作業の委譲、検証、統合を支援します。段階ごとに会話と承認を挟む実装は、対応するSkillを明示して使います。委譲の可否は実行環境とユーザーの指定に従います。

- `$agent-coordination:coordinate-subagent-work`: 独立した作業への担当範囲を定め、サブエージェントの結果を検証・統合する
