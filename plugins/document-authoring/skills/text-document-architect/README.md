...
このSkillは、文章中心の業務文書について、**中心命題・論証・根拠・認知的なメリハリ・情報配置・編集可能な成果物**を一つのワークフローとして扱います。

## この版の重要な方針

1. 本文は、対象読者が目的を達成するための最短かつ十分な読解経路にする。
2. 調査可能な事実とユーザーが所有する意思決定を分け、重要な判断だけを依存関係順に解消する。
3. 説明・断定だけを連続させず、読者の認知動作を切り替える。
4. 長文を一度で書き切らず、骨格、推論、根拠、メリハリ、圧縮の段階に分ける。
5. 候補情報を、本文、表／コールアウト、注記、Appendix、削除へ振り分ける。
6. 結論を変え得るリスクや前提をAppendixへ隠さない。
7. Word等では編集可能な原本を維持し、PDFやレンダリング画像は検品・配布に使う。
...
ルールを変更したときは、少なくとも次を確認します。

1. `SKILL.md`：実行手順
2. `references/quality-rubric.md`：合否基準
3. `references/artifact-production.md`：成果物生成の具体策
4. `evals/cases.md`：回帰防止
...
- **decision frontier**：前提となる判断が解決済みで、現在ユーザーが答えられる重要な意思決定の集合
