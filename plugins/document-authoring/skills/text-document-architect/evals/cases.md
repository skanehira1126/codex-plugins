...
依頼：「新サービスの社内提案書を作りたい。まだ対象読者と求める判断が曖昧なので、先に壁打ちして」

必須動作：

- 素材や調査から確認できる事実と、ユーザーが決める事項を分ける。
- 読者と求める判断など上流の意思決定を、依存関係上いま答えられるfrontierとして最大3件ずつ聞く。
- 各質問に推奨回答と理由を添え、回答前に下流の構成や本文へ進まない。
- frontierが空になったら共通理解を要約し、確認後に文書設計へ進む。

失敗条件：

- 素材から分かる事実をユーザーへ質問する。
- 上流判断が未決定のまま、網羅的な章構成や完全な本文を生成する。

### 安全な既定値で進められる通常依頼

依頼：「添付メモを、同じチーム向けの2ページのMarkdown報告書にして」

必須動作：

- 成果物を大きく変えない不足情報は安全な既定値として明示し、不要な質問をせず作成する。
- 明示的な壁打ち依頼がないため、仮想的な意思決定を掘り尽くさない。
...
- decision discoveryを、改訂の許可や新しい文書要件を得るために開始しない。
