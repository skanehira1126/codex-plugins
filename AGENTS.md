...
- `CHANGELOG.md`: marketplace全体のrelease noteの正本
...
5. 挙動を維持する重複統合と、発火条件、要件、出力、安全境界を変える意味変更は別フェーズにしてください。両方が依頼範囲なら、重複統合を先に検証してから意味変更へ進んでください。
6. ユーザーの未コミット変更を保持し、明示的な依頼なしに上書き、削除、巻き戻しをしないでください。
...
- 同じ指示を複数箇所に重複させないでください。削除前に詳細を所有する正本を決め、常に必要なroutingと重要な不変条件は `SKILL.md` に、状況依存の詳細は `references/` に置いてください。重複統合後は、残したcallerが正本へ到達することと、各層に固有のtrigger metadataや契約が失われていないことを確認してください。
...
- 配布するpluginの内容を変更した場合は、manifestのversionを更新し、rootの`CHANGELOG.md`へrelease noteを追加してください。挙動維持の整理はpatch、利用者向け挙動の変更は影響に合うversionとし、後者ではmanifestの説明と表示情報の更新要否も確認してください。
...
- 重複統合では、削除した詳細が正本に一度だけ残り、すべてのreference linkとresource callerが実在することを確認する。変更前後で発火条件、mode、出力契約、安全境界が変わっていないことも比較する。
