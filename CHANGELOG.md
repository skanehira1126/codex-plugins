...
### agent-coordination 0.5.0

- `coordinate-subagent-work`に起動時のモデル・推論強度・文脈の選択ガイドラインを追加した。モデル名は固定せず、ハーネスの「高速で手頃」「バランス型」などの説明と担当作業から必要十分な構成を選ぶ。再試行やレビューの負担も考慮し、具体的な推論上の必要に応じて強いモデルを使う。
- 親のモデルや高い推論強度の無条件継承を避け、短い引き継ぎと明示的な起動設定を優先する。モデル選択と全履歴forkの制約、強い構成を選ぶ理由も明記した。
- 基本方針を委譲する判断の幅に基づく選択へ絞り、`references/use-cases/data-science-and-analysis.md`へ分析タスクの分担・モデル説明・返す根拠の例を追加した。分析を委譲する場合だけ補足を読み込む。
- 委譲の発火条件、担当範囲、検証・統合と実行権限を維持し、表示情報と利用者向け文書を更新した。
